...
class AuthService:
    ...
    async def handle_signout(self, request: Request) -> Response:
        ...
        frontend_url = self.config.cors_origins[0] if self.config.cors_origins else "/"
        try:
            # Attempt to get user_id for logging, but don't fail if unable.
            user_id = None
            authorization = request.headers.get("Authorization")
            if authorization:
                scheme, token = authorization.split()
                if scheme.lower() == "bearer":
                    # Using validate_jwt for efficiency if it's a JWT
                    if len(token.split(".")) == 3:
                        try:
                            payload = await self.token_service.validate_jwt(token)
                            user_id = payload.get("sub")
                        except TokenError:
                             pass # Not a valid JWT, proceed without user_id
                    else: # Opaque token
                        try:
                            user_info = await self.token_service.get_user_info_from_token(token)
                            user_id = user_info.get("sub")
                        except TokenError:
                            pass # Not a valid opaque token, proceed without user_id
    ...
    async def get_user_info(self, request: Request, token_data: TokenData) -> UserInfo:
        # This method might now be less necessary if claims are sufficient.
        # But it can still be useful for fetching updated, non-claim user data.
        pass
